...
function createChromeMock() {
  ...
  const chrome = {
    ...
    storage: {
      ...
      onChanged: { addListener: vi.fn() } as Listener<(changes: Record<string, { newValue?: unknown }>, area: string) => void>,
      session: {
...
describe('background tab isolation', () => {
  ...
  it('does not create a browser tab group when the user disables grouping', async () => {
    const { chrome, tabs, groups } = createChromeMock();
    chrome.storage.local.get = vi.fn(async (key: string) => ({ [key]: key === 'opencli_disable_tab_group_v1' }));
    vi.stubGlobal('chrome', chrome);
