...
async function ensureOwnedContainerGroup(
  ...
  // Adapter automation already runs without visible groups. The user-facing
  // switch only suppresses the interactive browser container group.
  if (role === 'automation' || disableTabGroupCached) return null;
...
/**
 * After the last owned lease in a container window is released, close the
 * window instead of parking a blank placeholder tab in it. Only fires when
 * every remaining tab is disposable (blank), so a page the user opened in
 * that window by hand keeps the window alive.
 */
async function closeOwnedContainerIfDisposable(role: OwnedWindowRole, windowId: number | null): Promise<void> {
  ...
  try {
    ...
    // Window already gone or not queryable — nothing to close.
  }
}
  ...
  if (session.owned) {
    ...
    if (tabId !== null) {
      ...
      if (hasOtherOwnedLease) {
        ...
        // Last owned lease in this window: close the tab outright and take the
        // container window with it, rather than parking a blank placeholder
        // that lingers in the user's Chrome until the next command.
        await chrome.tabs.remove(tabId).catch(() => {});
