...
cli({
    ...
    args: [
        { name: 'path', required: true, positional: true, help: '要分享的路径，支持相对路径' },
    ...
    func: async (page, kwargs) => {
        requirePage(page);
        const filePath = resolvePath(kwargs.path);
        const expire = String(kwargs.expire || 86400);
        const pass = kwargs.pass || '';
        const extra = { expire };
        if (pass)
            extra.pass = pass;
