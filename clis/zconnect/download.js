...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        const outputDir = kwargs.output || path.join(os.homedir(), 'Downloads', 'zspace');
        const customName = kwargs.name || '';
        ...
        if (info.is_dir !== '1') {
            ...
            const result = await httpDownload(downloadUrl, destPath, {
                ...
                timeout: 600000,
                onProgress: (received, total) => { if (progressBar)
                    progressBar.update(received, total); },
            });
            if (progressBar)
                progressBar.complete(result.success);
